...
setting `parse_private` to `true` enables private (non-ICANN) domain parsing.
See the https://publicsuffix.org/list/public_suffix_list.dat[the public suffix list] for details on private/public domains.

NOTE: parse_private is a global filter setting.  If you use this tld filter multiple times in a filter, setting parse_private to true will make this setting true for all filter instances.
...
| <<plugins-{type}s-{plugin}-parse_private>> |<<boolean,boolean>>|No
|=======================================================================
...
[id="plugins-{type}s-{plugin}-parse_private"]
===== `parse_private`
